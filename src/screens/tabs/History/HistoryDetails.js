...
import { startRecording, stopRecording,requestPermission} from '../Message/voiceRecord_';
...
export default function HistoryDetails() {
  ...
  const session_history = useAppStore((s) => s.session_history);
  ...
  useEffect(() =>{
      console.log("session_history session- ", JSON.stringify(session_history, null, 2))
    if((session_history?.id)){
      setLoading(true);
      
      get_message_by_session_id(session_history?.id, (res, success) => {
        if(success){
